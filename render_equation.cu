...
__global__ void render_equation_forward_complex_kernel(
    ...
    const glm::vec3 *base_color, const float *roughness, const float *metallic,
    const glm::vec3 *normals, const glm::vec3 *viewdirs,
    const glm::vec3 *incidents_shs, const glm::vec3 *direct_shs,
    const float *visibility_shs, const int sample_num, glm::vec3 *incident_dirs,
    glm::vec3 *out_pbr, glm::vec3 *out_incident_lights,
    glm::vec3 *out_local_incident_lights, glm::vec3 *out_global_incident_lights,
    float *out_incident_visibility, glm::vec3 *out_diffuse_light,
    glm::vec3 *out_local_diffuse_light, float *out_accum, glm::vec3 *out_rgb_d,
    glm::vec3 *out_rgb_s) {
  ...
  for (int ray_id = 0; ray_id < sample_num; ray_id++) {
    ...
    z_s = {(1 + (-v33 - v22) / cos_p_1) * z_s.x +
               (-v3 + v12 / cos_p_1) * z_s.y + (v2 + v13 / cos_p_1) * z_s.z,
           (v3 + v12 / cos_p_1) * z_s.x + (1 + (-v33 - v11) / cos_p_1) * z_s.y +
               (-v1 + v23 / cos_p_1) * z_s.z,
           (-v2 + v13 / cos_p_1) * z_s.x + (v1 + v23 / cos_p_1) * z_s.y +
               (1 + (-v22 - v11) / cos_p_1) * z_s.z};
    const float norm =
        sqrtf(fmaxf(0.0000001f, z_s.x * z_s.x + z_s.y * z_s.y + z_s.z * z_s.z));
    ...
    float V = (0.5f / fmaxf(n_d_i * (1 - r2) + r2, 0.0000001f)) *
              (0.5f / fmaxf(n_d_o * (1 - r2) + r2, 0.0000001f));
...
void render_equation_forward_complex_cuda(
    ...
    const glm::vec3 *incidents_shs, const glm::vec3 *direct_shs,
    const float *visibility_shs, const int sample_num, glm::vec3 *incident_dirs,
    glm::vec3 *out_pbr, glm::vec3 *incident_lights,
    glm::vec3 *local_incident_lights, glm::vec3 *global_incident_lights,
    float *incident_visibility, glm::vec3 *diffuse_light,
    glm::vec3 *local_diffuse_light, float *accum, glm::vec3 *rgb_d,
    glm::vec3 *rgb_s) {
  render_equation_forward_complex_kernel<<<(P + 255) / 256, 256>>>(
      P, S_incident, S_direct, S_vis, base_color, roughness, metallic, normals,
      viewdirs, incidents_shs, direct_shs, visibility_shs, sample_num,
      incident_dirs, out_pbr, incident_lights, local_incident_lights,
      global_incident_lights, incident_visibility, diffuse_light,
      local_diffuse_light, accum, rgb_d, rgb_s);
}
...
RenderEquationForwardCUDA_complex(
    const torch::Tensor &base_color, const torch::Tensor &roughness,
    const torch::Tensor &metallic, const torch::Tensor &normals,
    const torch::Tensor &viewdirs, const torch::Tensor &incidents_shs,
    const torch::Tensor &direct_shs, const torch::Tensor &visibility_shs,
  ...
  torch::Tensor incident_dirs =
      torch::full({P, sample_num, 3}, 0.0, float_opts);

  torch::Tensor incident_lights =
      torch::full({P, sample_num, 3}, 0.0, float_opts);
  torch::Tensor local_incident_lights =
      torch::full({P, sample_num, 3}, 0.0, float_opts);
  torch::Tensor global_incident_lights =
      torch::full({P, sample_num, 3}, 0.0, float_opts);
  torch::Tensor incident_visibility =
      torch::full({P, sample_num, 1}, 0.0, float_opts);
  ...
  render_equation_forward_complex_cuda(
      ...
      visibility_shs.contiguous().data_ptr<float>(), sample_num,
  ...
  return std::make_tuple(pbr, incident_dirs, incident_lights,
                         local_incident_lights, global_incident_lights,
                         incident_visibility, diffuse_light,
                         local_diffuse_light, accum, rgb_d, rgb_s);
}
...
__global__ void render_equation_backward_kernel(
    ...
    const glm::vec3 *normals, const glm::vec3 *viewdirs,
    const glm::vec3 *incidents_shs, const glm::vec3 *direct_shs,
    const float *visibility_shs, const int sample_num,
    const glm::vec3 *incident_dirs, const glm::vec3 *dL_dpbrs,
    const glm::vec3 *dL_ddiffuse_lights, glm::vec3 *dL_dbase_color,
    float *dL_droughness, float *dL_dmetallic, glm::vec3 *dL_dnormals,
    glm::vec3 *dL_dviewdirs, glm::vec3 *dL_dincidents_shs,
    glm::vec3 *dL_ddirect_shs, float *dL_dvisibility_shs) {
  auto idx = cg::this_grid().thread_rank();
  if (idx >= P)
    return;
  const glm::vec3 normal = normals[idx];
  ...
  for (int ray_id = 0; ray_id < sample_num; ray_id++) {
    ...
    glm::vec3 incident_light =
        incident_visibility * global_incident_light + local_incident_light;
    ...
    glm::vec3 f_s = D * F * V;
    // pbr += (f_d+f_s) * incident_light * (2.0f * 3.14159f * n_d_i /
    // (float)sample_num);

    glm::vec3 dL_dfd = dL_dpbr * incident_light *
                       (2.0f * 3.14159f * n_d_i / (float)sample_num);
    glm::vec3 dL_dfs = dL_dpbr * incident_light *
                       (2.0f * 3.14159f * n_d_i / (float)sample_num);
    glm::vec3 dL_dincident_light =
        dL_dpbr * (f_d + f_s) * (2.0f * 3.14159f * n_d_i / (float)sample_num);
    float dL_dn_d_i = glm::dot(dL_dpbr, (f_d + f_s) * incident_light) *
                      (2.0f * 3.14159f / (float)sample_num);

    // from dL_ddiffuse_light
    dL_dincident_light +=
        dL_ddiffuse_light * (2.0f * 3.14159f * n_d_i / (float)sample_num);
    dL_dn_d_i +=
        glm::dot(dL_ddiffuse_light,
                 incident_light * (2.0f * 3.14159f / (float)sample_num));
    ...
    float dL_dr2 =
        -2.0f / (r2 * r2) * dL_dsharp - 1.0f / (r2 * r2 * 3.14159f) * dL_damp;
    float dL_drough = dL_dr2 * 2.0f * rough;
    // from dL_dF
    glm::vec3 dL_dF0 = (1.0f - powf(1.0f - h_d_o, 5.0f)) * dL_dF;
    float dL_dh_d_o =
        glm::dot((1.0f - F_0), dL_dF) * -5.0f * powf(1.0f - h_d_o, 4.0f);
    ...
    glm::vec3 dL_dglobal_incident_light =
        dL_dincident_light * incident_visibility;
    float dL_dincident_visibility =
        glm::dot(dL_dincident_light, global_incident_light);
    if (incident_visibility <= 1.0f && incident_visibility >= 0.0f) {
      for (int i = 0; i < S_vis; i++) {
        dL_dvisibility_shs[idx * S_vis + i] +=
            dL_dincident_visibility * coefs[i];
      }
    }
    if (global_incident_light.x < 0.0f)
      dL_dglobal_incident_light.x = 0.0f;
    if (global_incident_light.y < 0.0f)
      dL_dglobal_incident_light.y = 0.0f;
    if (global_incident_light.z < 0.0f)
      dL_dglobal_incident_light.z = 0.0f;
    for (int i = 0; i < S_direct; i++) {
      dL_ddirect_shs[i] += dL_dglobal_incident_light * coefs[i];
    }

    if (local_incident_light.x < 0.0f)
      dL_dlocal_incident_light.x = 0.0f;
    if (local_incident_light.y < 0.0f)
      dL_dlocal_incident_light.y = 0.0f;
    if (local_incident_light.z < 0.0f)
      dL_dlocal_incident_light.z = 0.0f;
    for (int i = 0; i < S_direct; i++) {
      dL_dincidents_shs[idx * S_incident + i] +=
          dL_dlocal_incident_light * coefs[i];
    }
...
void render_equation_backward_cuda(
    ...
    const float *visibility_shs, const int sample_num,
    const glm::vec3 *incident_dirs, const glm::vec3 *dL_dpbrs,
    const glm::vec3 *dL_ddiffuse_light, glm::vec3 *dL_dbase_color,
    float *dL_droughness, float *dL_dmetallic, glm::vec3 *dL_dnormals,
    glm::vec3 *dL_dviewdirs, glm::vec3 *dL_dincidents_shs,
    glm::vec3 *dL_ddirect_shs, float *dL_dvisibility_shs) {
  render_equation_backward_kernel<<<(P + 255) / 256, 256>>>(
      P, S_incident, S_direct, S_vis, base_color, roughness, metallic, normals,
      viewdirs, incidents_shs, direct_shs, visibility_shs, sample_num,
      incident_dirs, dL_dpbrs, dL_ddiffuse_light, dL_dbase_color, dL_droughness,
      dL_dmetallic, dL_dnormals, dL_dviewdirs, dL_dincidents_shs,
      dL_ddirect_shs, dL_dvisibility_shs);
}
...
RenderEquationBackwardCUDA(
    ...
    const torch::Tensor &viewdirs, const torch::Tensor &incidents_shs,
    const torch::Tensor &direct_shs, const torch::Tensor &visibility_shs,
    const int sample_num, const torch::Tensor &incident_dirs,
    const torch::Tensor &dL_dpbrs, const torch::Tensor &dL_ddiffuse_light,
    const bool debug) {
  const int P = base_color.size(0);
  const int S_incident = incidents_shs.size(1);
  const int S_direct = direct_shs.size(1);
  ...
  torch::Tensor dL_dincidents_shs =
      torch::zeros({P, S_incident, 3}, float_opts);
  ...
  return std::make_tuple(dL_dbase_color, dL_droughness, dL_dmetallic,
                         dL_dnormals, dL_dviewdirs, dL_dincidents_shs,
                         dL_ddirect_shs, dL_dvisibility_shs);
}
...
__global__ void render_equation_forward_kernel(
    ...
    const glm::vec3 *normals, const glm::vec3 *viewdirs,
    const glm::vec3 *incidents_shs, const glm::vec3 *direct_shs,
    const float *visibility_shs, const int sample_num, const bool is_training,
    const float *rand_float, glm::vec3 *incident_dirs, glm::vec3 *out_pbr,
  ...
  for (int ray_id = 0; ray_id < sample_num; ray_id++) {
    ...
    float theta = delta * ray_id;
    if (is_training)
      theta = rand_float[whole_idx] * 2 * 3.14159f + theta;
    const float y = cosf(theta) * rad;
    const float x = sinf(theta) * rad;
    float3 z_s = {x, y, z};
    ...
    glm::vec3 f_s = D * F * V;
    glm::vec3 transport =
        incident_light * (2.0f * 3.14159f * n_d_i / (float)sample_num);
...
void render_equation_forward_cuda(
  ...
  render_equation_forward_kernel<<<(P + 255) / 256, 256>>>(
      P, S_incident, S_direct, S_vis, base_color, roughness, metallic, normals,
      viewdirs, incidents_shs, direct_shs, visibility_shs, sample_num,
      is_training, rand_float, incident_dirs, out_pbr, out_diffuse_light);
}
...
RenderEquationForwardCUDA(
    ...
    const torch::Tensor &viewdirs, const torch::Tensor &incidents_shs,
    const torch::Tensor &direct_shs, const torch::Tensor &visibility_shs,
    const int sample_num, const bool is_training, const bool debug) {
  const int P = base_color.size(0);
  const int S_incident = incidents_shs.size(1);
  const int S_direct = direct_shs.size(1);
  const int S_vis = visibility_shs.size(1);
  ...
  render_equation_forward_cuda(
      ...
      (glm::vec3 *)incidents_shs.contiguous().data_ptr<float>(),
      (glm::vec3 *)direct_shs.contiguous().data_ptr<float>(),
      visibility_shs.contiguous().data_ptr<float>(), sample_num, is_training,
